...
function processNanaQuest() {
    if (cm.haveItem(questItems[nanaLoc], 50)) {
        if (cm.canHold(4031367 + nanaLoc, 1)) {
            ...
            cm.sendOk("呀~ 非常感谢，给你#b#t4031367##k。");
            return true;
        } else {
            cm.sendOk("请确保有一个空余的其他栏来存放爱的证明。");
        }
    } else {
        cm.sendOk("请给我 #b50个#t" + questItems[nanaLoc] + "##k。");
    }
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (!cm.isQuestStarted(100400)) {
                cm.sendOk("你好 #b#h0##k，我是爱情精灵 #p9201023#。");     // thanks Periwinks (LuckyStory) for noticing Nana's introducing themselves as Nana (H)
                cm.dispose();
                return;
            }
            ...
            if (nanaLoc == -1) {
                cm.sendOk("你好 #b#h0##k，我是爱情精灵 #p9201023#。");
                cm.dispose();
                return;
            }
            ...
            if (!cm.haveItem(4031367 + nanaLoc, 1)) {
                ...
                cm.sendOk("嘿，你好。你已经得到了#t4031367#了吗？");
                cm.dispose();
            }
        } else if (status == 1) {
            if (state == 0) {
