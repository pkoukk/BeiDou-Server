...
function start() {
    ...
    var options = ["项链","脸饰","眼饰","腰带&勋章","戒指"/*,"#t4032496#"*/];
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selectedType == 0) { // 项链 pendant refine
            ...
            var matQtySet = [[20, 20, 10, 10], [10, 10, 20, 2], [10, 1, 10, 4], [10, 1, 10, 4], [15, 1, 15, 6], [15, 1, 15, 6], [20, 1, 20, 8], [20, 1, 20, 8], [1, 2, 3]];
            var costSet = [500000, 150000, 30000, 30000, 40000, 40000, 50000, 50000, 2500000];
        } else if (selectedType == 1) { // 脸饰 face accessory refine
            ...
            var matQtySet = [[10, 10], [15, 15, 15], [20, 20, 20, 10, 2], [10, 10], [15, 15, 15], [20, 20, 20, 10, 2], [3, 1], [3, 1], [3, 1], [3, 1]];
            var costSet = [100000, 200000, 300000, 125000, 250000, 375000, 50000, 50000, 50000, 50000, 25000, 25000, 25000, 25000];
        } else if (selectedType == 2) { // 眼饰 eye accessory refine
            ...
            var matQtySet = [[4, 4, 10, 20], [3, 2], [5, 4], [7, 5, 20], [2, 2, 10, 10]];
            var costSet = [250000, 250000, 300000, 400000, 200000];
        } else if (selectedType == 3) { // 腰带和勋章 belt & medals refine
            var matSet = [[4001006, 4003005, 4003004], [7777, 7777]];
            var matQtySet = [[5, 20, 20], [7777, 7777]];
            var costSet = [1000000, 7777];
        } else if (selectedType == 4) { // 戒指 ring refine
            ...
            var matQtySet = [[5, 5, 5], [5, 5, 5], [2, 1], [3, 5], [1, 10], [1, 5], [1, 1]];
            var costSet = [100000, 100000, 100000, 200000, 250000, 300000, 100000];
        ...
        if (mats instanceof Array) {
            for (var i = 0; i < mats.length; i++) {
                prompt += "\r\n#i" + mats[i] + "# " + (matQty[i] * qty) + "个 #z" + mats[i] + "#";
            }
        } else {
            prompt += "\r\n#i" + mats + "# " + (matQty * qty) + "个 #z" + mats + "#";
        }
        if (cost > 0) {
        ...
        if (cm.getMeso() < (cost * qty)) {
            cm.sendOk("这是我制作物品所收取的费用！不接受信用卡。");
        } else {
            var complete = true;
            if (mats instanceof Array) {
            ...
            if (!complete) {
                cm.sendOk("你确定你拿齐了所有需要的物品吗？再检查一遍吧！");
            } else {
