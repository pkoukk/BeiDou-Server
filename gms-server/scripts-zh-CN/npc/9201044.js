    ...
    if (mode == -1) {
        ...
        if (eim.getProperty(stage.toString() + "stageclear") != null) {
            ...
            if (eim.isEventLeader(cm.getPlayer())) {
                ...
                if (state == -1) {           // preamble
                    if (stage == 1) {
                        cm.sendOk("嗨。欢迎来到婚礼村挑战的第#b" + stage + "#k关。在这个阶段，与#p9201047#交谈，他会向你传达任务的进一步细节。在打碎下面的魔镜后，将碎片交给#p9201047#，然后来这里获得进入下一个阶段的权限。");
                    } else if (stage == 2) {
                        cm.sendOk("嗨。欢迎来到婚礼村挑战的第#b" + stage + "#k关。在这个阶段，让你的5名队员以某种方式爬上平台，尝试组合解锁通往下一级的传送门。当你感觉准备好了，和我交谈，我会告诉你情况。然而，请做好准备，如果传送门在几次尝试后没有解锁，怪物将会生成。");
                    } else if (stage == 3) {
                        cm.sendOk("嗨。欢迎来到婚礼村挑战的第#b" + stage + "#k关。在这个阶段，让你的5名队员分别爬上平台，尝试组合以解锁通往下一级的传送门。当你准备好时，和我交谈，我会告诉你情况。提示：失败时，数一下场景中出现的史莱姆数量，这将告诉你有多少人的位置是正确的。");
                    }
                    ...
                    if (stage == 1) {
                        ...
                        if (map.countMonsters() == 0) {
                            ...
                            if (playersOnCombo == 5/* || cm.getPlayer().gmLevel() > 1*/ || debug) {
                                ...
                                if (stage == 2 && eim.getPlayers().size() == 6) {  // 检测到队伍人数不足时，直接通过本关，方便单机玩家
                                    cm.sendNext("看起来你们还没有找到这个试炼的方法。考虑一下在绳子上安排5名成员。记住，只允许有5人挂在在绳子上，如果你移动了，可能就不算作答案了，所以请记住这一点。继续努力！");
                                } else if (stage == 2 &&eim.getPlayers().size() < 6) {  // 检测到队伍人数不足时，直接通过本关，方便单机玩家
     		  eim.showClearEffect(true);
       		  eim.linkToNextStage(stage, "apq", curMap);  //opens the portal to the next ma

                                } else if (stage == 3 &&eim.getPlayers().size() == 6) {  // 检测到队伍人数不足时，直接通过本关，方便单机玩家
