...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        if (cm.getMapId() == 910320001) {
            ...
            cm.sendSimple("我的名字是#p1052115#。\r\n#b#e#L1#进入尘土飞扬的平台。#l#n\r\n#L2#前往999号列车。#l\r\n#L3#获得<#z1142141#>。#l#k");
        }
    } else if (status == 2) {
        section = selection;
        if (selection == 1) {
            if (cm.getPlayer().getLevel() < 25 || cm.getPlayer().getLevel() > 255 || !cm.isLeader()) {
                cm.sendOk("你必须处于25-255级的等级范围，并且是队伍的队长。");
            } else {
                //if (!cm.start_PyramidSubway(-1)) {  //缺少完整脚本
		...
		//}
            ...
            if (cm.haveItem(4001321)) {
                //if (cm.bonus_PyramidSubway(-1)) { //缺少完整脚本
            ...
            if (mons < 10000) {
                cm.sendOk("请在站点击败至少10,000只怪物，然后再来找我。当前击杀数量 : " + mons);
