...
function action(mode, type, selection) {
	...
	if (cm.getPlayerCount(802000111) <= 1) {//BOSS地图无人，逆奥BOSS暂不能重连，因此允许玩家掉线后点NPC进入，改为0则不允许
		...
		if (party == null) {
			...
			if (party.getLeaderId() != player.getId()) {
				...
				//for (var i = 0; i < members.size(); i++) {
					...
					//if (chr.getQuestStatus(4698) != 2) { //前置任务ID待确认
					//	canGoIn = false;
					//	cause = chr.getName() + "没完成前置任务,无法进入";
					//	break;
					//}
					//if (chr.getBossLog(0, "挑战努克斯") >= timeLimit) {  //次数限制暂无法使用
					//	canGoIn = false;
					//	cause = chr.getName() + "玩家的挑战次数不足,无法进入";
					//	break;
					//}
				//}
				if (canGoIn &&cm.getPlayerCount(802000111) <=0) { //地图中没人的话就重置地图
					...
					//	members.get(i).setBossLog(0, "挑战努克斯");
					//}
					return true;
                        		} else if (canGoIn &&cm.getPlayerCount(802000111) >0){ //地图中有人的话就不重置
                            		var Map1 = cm.getMap(802000111);
                           	 	cm.warpParty(802000111, 0);
                            		members = player.getPartyMembersOnSameMap();
                           	 	//for (var i = 0; i < members.size(); i++) {   //记录队员挑战次数，但次数限制暂无法使用
