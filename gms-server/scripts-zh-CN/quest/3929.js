...
function end(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if(c == 4 && !qm.haveItem(4031580)) {
                //qm.sendNext("你把所有的食物都运送了，很好。");
                qm.sendNext("你把所有的食物都运送了，很好。(任务有bug，可丢弃物品后直接完成)");
                qm.gainExp(2000);
                ...
                if(missed > 0) {
                    if(qm.canHold(4031580, missed)) {
                        ...
                        qm.sendNext("你没有完成任务，背包里也没有空位来取食物。请在背包其他栏腾出一个空位。");
                    }
                } else {
                    qm.sendNext("嘿，你想干什么？要通过我的测试，你必须把所有的食物送到居民区。");
                }
            }
